...
class FileView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        #Cambios por Uriel Corral

        json_report = ast.literal_eval(malware_obj.virus_total)

        tag_list = db.list_tags_for_malware(sha256)
        children = db.list_children(malware_obj.id)
        parent = db.get_parent(malware_obj.id)

        return render(request, template_name, {'report': json_report,
                                                'malware': malware_obj,
                                               'note_list': note_list,
                                               'tag_list': tag_list,
                                               'children': children,
